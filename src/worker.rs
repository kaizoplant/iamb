            ...
            for (msg, receipts) in msgs.into_iter() {
                ...
                match msg {
                    ...
                    AnyTimelineEvent::MessageLike(AnyMessageLikeEvent::RoomMessage(msg)) => {
                        info.insert_with_preview(msg, settings, need_load, previews, worker);
                    },
...
fn events_insert(
    ...
    if let Ok(events) = res {
        let ChatStore { rooms, worker, previews, settings, need_load, .. } =
            &mut locked.application;
        let info = rooms.get_or_default(room_id.clone());
            ...
            |ev: SyncMessageLikeEvent<RoomMessageEventContent>,
             ...
             store: Ctx<AsyncProgramStore>| {
                async move {
                    ...
                    info.insert_with_preview(full_ev, settings, need_load, previews, worker);
                }
            },
        );

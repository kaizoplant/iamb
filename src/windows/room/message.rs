...
impl MessageState {
    ...
    fn render<'b>(
        ...
        for (row, mut line) in txt.lines.into_iter().enumerate() {
            ...
            let line_preview: Vec<_> = msg_previews
        ...
        if settings.tunables.reaction_display {
            for (key, users, source) in info.get_reactions(&self.message_id) {
        ...
        for y in draw_area.top()..draw_area.top() + draw_area.height {
            ...
            if let Some((line_style, styles, line_preview)) = draw_lines.next() {
                image_previews.extend(
                    line_preview.into_iter().map(|(backend, msg_x)| (x + msg_x, y, backend)),
                );
